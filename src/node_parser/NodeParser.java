...
public class NodeParser {
    ...
    public Node sum() throws IOException {
        ...
        while (lexer.token.getSymbol() == Lexer.LexerValues.PLUS ||
                ...
                lexer.token.getSymbol() == Lexer.LexerValues.LESS ||
                lexer.token.getSymbol() == Lexer.LexerValues.MORE) {
            if (lexer.token.getSymbol() == Lexer.LexerValues.PLUS) {
                type = Node.NodeType.ADD;
            } else if (lexer.token.getSymbol() == Lexer.LexerValues.MINUS){
                type = Node.NodeType.SUB;
            }else if (lexer.token.getSymbol() == Lexer.LexerValues.LESS){
                type = Node.NodeType.LESS;
            } else {
                type = Node.NodeType.MORE;
            }
            lexer.nextToken();
            Node op1 = n;
            n = new Node();
